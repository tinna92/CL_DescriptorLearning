...
Affine_patch_dir = dir([Imgpath '*Affine_featurePatch.mat']);

Pair_index_first_dir = dir([Imgpath '*pair_index_first.mat']);
Pair_index_second_dir = dir([Imgpath '*pair_index_second.mat']);
nAffine_patch_dir = numel(Affine_patch_dir);
nPair_index_first_dir = numel(Pair_index_first_dir);   
nPair_index_second_dir = numel(Pair_index_second_dir);

NNN = 0;
affine_patch_pos = cell(431691,2);
for iFile = 1:nAffine_patch_dir
%     load([Imgpath Affine_patch_dir(iFile).name]);
    load([Imgpath Pair_index_first_dir(iFile).name]);
    load([Imgpath Pair_index_second_dir(iFile).name]);
    FileNum = numel(pair_index_first);
    for iii=1:FileNum
        Current_num = numel(pair_index_first{iii});        
        patchpair_image = imread([Imgpath D(iFile).name '-' int2str(iii) '-' 'pair.jpg']); % read the pair images
        for jjj=1:Current_num
            col_num = mod(jjj,16);
            row_num = mod(jjj,16);
            ...
            affine_patch_pos{NNN+jjj,1} =  imresize(patchpair_image(orig_left:orig_right,orig_top:orig_bottom),[32 32]);
            affine_patch_pos{NNN+jjj,2} = imresize(patchpair_image(orig_right+1:orig_right+63,orig_top:orig_bottom),[32 32]);  
        end
        NNN = NNN + Current_num;
    end    
end

save([Imgpath D(iFile).name '-' int2str(iii) '-' 'affine_patch_pos.mat'],'affine_patch_pos');
% nFiles = numel(D);
% for iFile = 1:nFiles        
