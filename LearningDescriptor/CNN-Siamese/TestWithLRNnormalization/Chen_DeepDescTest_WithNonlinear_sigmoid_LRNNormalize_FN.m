...
function [Eucl_dist]= Chen_DeepDescTest_WithNonlinear_sigmoid_LRNNormalize_FN(theta,shrinkRate,x_l,x_r,y,l_pull,l_push)
sizew1 = [5 5 1 5];
sizeb1 = [1 5];
sizew2 = [5 5 5 25];
sizeb2 = [1 25];
...
MM = size(resl.x7,3);
NN = size(resl.x7,4);
xNNl = reshape(resl.x7,[MM NN]);
xNNr = reshape(resr.x7,[MM NN]);
