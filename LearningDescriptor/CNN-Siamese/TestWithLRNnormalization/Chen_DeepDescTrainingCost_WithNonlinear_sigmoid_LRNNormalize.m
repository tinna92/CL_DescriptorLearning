...
function [cost,grad]= Chen_DeepDescTrainingCost_WithNonlinear_sigmoid_LRNNormalize(theta,shrinkRate,x_l,x_r,y,l_pull,l_push)
sizew1 = [5 5 1 5];
sizeb1 = [1 5];
sizew2 = [5 5 5 25];
sizeb2 = [1 25];
...
[resl,resr] = PatchDesc_DeepCnn_WithNonlinear_sigmoid_LRNNormalize(x_l,x_r, w1, b1,w2,b2,w3,b3);
...
% Eucl_dist(t) = sqrt(sum(dist(:,t))); %changed on 12.05.2016 to cope with
% trianing better
Eucl_dist(t) = sum(dist(:,t));
...
dLdxl = single(dLdxl);
dLdxr = single(dLdxr);
[resl,resr] = PatchDesc_DeepCnn_WithNonlinear_sigmoid_LRNNormalize(x_l,x_r, w1, b1,w2,b2,w3,b3,dLdxl,dLdxr);
