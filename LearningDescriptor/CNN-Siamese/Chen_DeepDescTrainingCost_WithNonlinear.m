...
function [cost,grad]= Chen_DeepDescTrainingCost_WithNonlinear(theta,shrinkRate,x_l,x_r,y,l_pull,l_push)
sizew1 = [5 5 1 5];
sizeb1 = [1 5];
sizew2 = [5 5 5 25];
sizeb2 = [1 25];
...
[resl,resr] = PatchDesc_DeepCnn_WithNonlinear(x_l,x_r, w1, b1,w2,b2,w3,b3);
...
%           L = y.*(max(0,Eucl_dist-1.5))+3*(1-y).*(max(0,4.5-Eucl_dist))+ 0.5 * shrinkRate * sum(w1(:).^2)+0.5 * shrinkRate * sum(w2(:).^2) +0.5 * shrinkRate * sum(w3(:).^2);% Try to change the loss function into L1 Norm
          L = 0.5*y.*(max(0,Eucl_dist-l_pull).^2)+0.5*(1-y).*(max(0,l_push-Eucl_dist).^2)+ 0.5 * shrinkRate * sum(w1(:).^2)+0.5 * shrinkRate * sum(w2(:).^2) +0.5 * shrinkRate * sum(w3(:).^2);% change the loss function into square of Euclidean distance
