...
function [Eucl_dist]= Chen_DeepDescTest_WithNonlinear(theta,shrinkRate,x_l,x_r,y,l_pull,l_push)
sizew1 = [5 5 1 5];
sizeb1 = [1 5];
sizew2 = [5 5 5 25];
sizeb2 = [1 25];
...
L = 0.5*y.*(max(0,Eucl_dist-l_pull).^2)+0.5*(1-y).*(max(0,l_push-Eucl_dist).^2)+ ...
    0.5 * shrinkRate * sum(w1(:).^2)+0.5 * shrinkRate * sum(w2(:).^2) +0.5 * shrinkRate * sum(w3(:).^2);% change the loss function into square of Euclidean distance
% Loss = L;
end