...
alpha =  0.01;
% alpha =  0.003;
alpha_decreaserate_eachEpoch = 0.999;
...
for ii=1:Size_trainingSamples
    x_pos{ii,1} = notredame_x_pos{Training_sampleIndex_pos(ii),1};
    x_pos{ii,2} = notredame_x_pos{Training_sampleIndex_pos(ii),2};
    x_neg{ii,1} = notredame_x_neg{Training_sampleIndex_neg(ii),1};
    x_neg{ii,2} = notredame_x_neg{Training_sampleIndex_neg(ii),2};
end

% % calculate the input for validation set
for ii=1:Size_validationSamples
    test_x_l(:,:,1,ii) = single(notredame_x_pos{Validation_SampleIndex_pos(ii),1});
    test_x_r(:,:,1,ii) = single(notredame_x_pos{Validation_SampleIndex_pos(ii),2});
    test_x_l(:,:,1,ii+Size_validationSamples) = single(notredame_x_neg{Validation_SampleIndex_neg(ii),1});
    test_x_r(:,:,1,ii+Size_validationSamples) = single(notredame_x_neg{Validation_SampleIndex_neg(ii),2});
end
...
valid_test_y(Size_validationSamples+1:2*Size_validationSamples)=0;


for ii=1:Size_trainingSamples
    x_l_all(:,:,1,ii) = single(notredame_x_pos{Training_sampleIndex_pos(ii),1});
    x_r_all(:,:,1,ii) = single(notredame_x_pos{Training_sampleIndex_pos(ii),2});
    x_l_all(:,:,1,ii+Size_trainingSamples) = single(notredame_x_neg{Training_sampleIndex_neg(ii),1});
    x_r_all(:,:,1,ii+Size_trainingSamples) = single(notredame_x_neg{Training_sampleIndex_neg(ii),2});
end


% central normalize the training data 
for i=1:Size_trainingSamples*2
    tmp_xl =  x_l_all(:,:,:,i);
    tmp_xr =  x_r_all(:,:,:,i);
    x_l_norm(:,:,:,i) = (tmp_xl-mean(mean(mean(tmp_xl))))/std(tmp_xl(:));
    x_r_norm(:,:,:,i) = (tmp_xr-mean(mean(mean(tmp_xr))))/std(tmp_xr(:));
%     x_l_pos(:,:,:,i) = (x_l(:,:,:,i) - mean(mean(mean(x_l(:,:,:,i)))))/std(tmp_xl(:));
%     x_r_neg(:,:,:,i) = (x_r(:,:,:,i) - mean(mean(mean(x_r(:,:,:,i)))))/std(tmp_xr(:));
end
...
%         y(Size_batches+1:2*Size_batches)=0;
        ...
        for ii=1:Size_batches
            x_l(:,:,1,ii) = x_l_norm(:,:,1,SampleIndex_pos((batch_ground-1)*Size_batches+ii));
            x_r(:,:,1,ii) = x_r_norm(:,:,1,SampleIndex_pos((batch_ground-1)*Size_batches+ii));
            x_l(:,:,1,ii+Size_batches) = x_l_norm(:,:,1,SampleIndex_neg((batch_ground-1)*Size_batches+ii)+Size_trainingSamples);
            x_r(:,:,1,ii+Size_batches) = x_r_norm(:,:,1,SampleIndex_neg((batch_ground-1)*Size_batches+ii)+Size_trainingSamples);
        end
...
%         [cost,grad_I]= Chen_DeepDescTrainingCost_WithNonlinear_sigmoid_LRNNormalize(theta,shrinkRate,x_l,x_r,y,0,10); % test with nonlinear here
       
        r_tminus1 = r_t; % r(t-1) = last r(t)
        theta_tplusahalf = theta - beta_momentum.*v_t;
        [cost,grad_theta_tplushalf]= Chen_DeepDescTrainingCost_WithNonlinear_sigmoid_LRNNormalize(theta_tplusahalf,shrinkRate,x_l,x_r,y,0,10);
...
%     hold on;
    ...
    [cost_Adjust,grad_theta_tplushalf]= Chen_DeepDescTrainingCost_WithNonlinear_sigmoid_LRNNormalize(theta,shrinkRate,test_x_l,test_x_r,valid_test_y,0,10);
    validation_test_cost(iii) = mean(cost_Adjust);
    Theta_record{iii} = theta;
...
load('liberty_x_neg.mat');
load('liberty_x_pos.mat');
x_neg_test = liberty_x_neg;
x_pos_test = liberty_x_pos;
...
% clear TPR FPR
save('learnedParamaters0406_80_batchsize_1000_notredame_validation.mat','theta');
save('Save_v_t_r_t_Loss_alpha_0406_80_batchsize_1000_notredame_validation.mat','r_t','v_t','Loss_Rec','alpha');
