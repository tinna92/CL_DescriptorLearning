...
function [resl,resr] = PatchDesc_DeepCnn_WithNonlinear_sigmoid_LRNN_TANH(x_l,x_r, w1, b1,w2,b2,w3,b3, dzdxl,dzdxr)
...
% if nargin <= 8
    ...
    resl.x2n = vl_nntanh(resl.x2);% increase a nonlinear layer
%     vl_nnsigmoid(x,dzdy);
    ...
    resl.x4n = vl_nntanh(resl.x4);% increase a nonlinear layer
...
%     resl.x7 = vl_nnnormalize(resl.x6,[5 2 1.0000e-04 0.75]);
    
    resr.x1 = x_r ;
    resr.x2 = vl_nnconv(resr.x1, w1, b1) ;
    resr.x2n = vl_nntanh(resr.x2);% increase a nonlinear layer
    ...
    resr.x4n = vl_nntanh(resr.x4);% increase a nonlinear layer
...
if nargin > 8
    ...
    resl.dzdx4 = vl_nntanh(resl.x4,resl.dzdx4n);
    ...
    resl.dzdx2 = vl_nntanh(resl.x2,resl.dzdx2n);
    ...
    resr.dzdx4 = vl_nntanh(resr.x4, resr.dzdx4n);
%     resr.dzdx4 = vl_nnpool(resr.x4, rho2,'Stride',2,resr.dzdx5);
    ...
    resr.dzdx2 = vl_nntanh(resr.x2,resr.dzdx2n);
