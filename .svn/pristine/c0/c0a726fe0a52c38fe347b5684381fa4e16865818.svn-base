...
% -------------------------------------------------------------------------
Num_Epoches  = 100; % num of iterations( for each training sample)
...
x_pos = cell(Size_trainingSamples*2,2);
x_neg = cell(Size_trainingSamples*2,2);
...
Training_sampleIndex_neg = SampleIndex_neg(Size_validationSamples+1:Size_allSamples);
for ii=1:Size_trainingSamples
    x_pos{ii+Size_trainingSamples,1} = yosemite_x_pos{Training_sampleIndex_pos(ii),1};
    x_pos{ii+Size_trainingSamples,2} = yosemite_x_pos{Training_sampleIndex_pos(ii),2};
    x_neg{ii+Size_trainingSamples,1} = yosemite_x_neg{Training_sampleIndex_neg(ii),1};
    x_neg{ii+Size_trainingSamples,2} = yosemite_x_neg{Training_sampleIndex_neg(ii),2};
end
...
for ii=1:Size_validationSamples
    test_x_l(:,:,1,ii+2*Size_validationSamples) = single(yosemite_x_pos{Validation_SampleIndex_pos(ii),1});
    test_x_r(:,:,1,ii+2*Size_validationSamples) = single(yosemite_x_pos{Validation_SampleIndex_pos(ii),2});
    test_x_l(:,:,1,ii+3*Size_validationSamples) = single(yosemite_x_neg{Validation_SampleIndex_neg(ii),1});
    test_x_r(:,:,1,ii+3*Size_validationSamples) = single(yosemite_x_neg{Validation_SampleIndex_neg(ii),2});
end

for i=1:Size_validationSamples*4
    tmp_xl =  test_x_l(:,:,:,i);
    tmp_xr =  test_x_r(:,:,:,i);
...
valid_y(2*Size_validationSamples+1:3*Size_validationSamples)=-1;
valid_y(3*Size_validationSamples+1:4*Size_validationSamples)=1;
valid_test_y(1:Size_validationSamples)=-1;
valid_test_y(Size_validationSamples+1:2*Size_validationSamples)=1;
valid_test_y(2*Size_validationSamples+1:3*Size_validationSamples)=-1;
valid_test_y(3*Size_validationSamples+1:4*Size_validationSamples)=1;
Num_train_batches = 2*Num_train_batches;

for iii=1:Num_Epoches
    
    SampleIndex_pos = randperm(Size_trainingSamples*2,Size_trainingSamples*2); % used to generate the sample indexes
    SampleIndex_neg = randperm(Size_trainingSamples*2,Size_trainingSamples*2); % used to generate the sample indexes   
    ...
    for batch_ground =1:Num_train_batches 
        ...
        [cost,grad]= Chen_DeepDescTrainingCost_WithNonlinear_sigmoid_LRNNormalize(theta,shrinkRate,x_l,x_r,y,3,10); % test with nonlinear here
        Loss_Rec_batch(batch_ground) = mean(cost);
        fprintf('%d - %f  ',batch_ground,Loss_Rec_batch(batch_ground));

        
        [AA1 AA_index1] = sort(cost(1:Size_batches),'descend');
        [AA2 AA_index2] = sort(cost(Size_batches+1:2*Size_batches),'descend');
        
        Min_Select_index = [AA_index1(1:125); AA_index2(1:125)+ Size_batches]; % index for mining
        x_l_Adjust = x_l(:,:,:,Min_Select_index);
        x_r_Adjust = x_r(:,:,:,Min_Select_index);
        y_adjust = y(Min_Select_index);
        [cost_Adjust,grad]= Chen_DeepDescTrainingCost_WithNonlinear_sigmoid_LRNNormalize(theta,shrinkRate,x_l_Adjust,x_r_Adjust,y_adjust,3,10); % test with nonlinear here

        
        % this comment part code is used to conduct the rmsprop with
%         nestrov gradient descent.
        r_tminus1 = r_t; % r(t-1) = last r(t)
        theta_tplusahalf = theta - beta_momentum.*v_t;
        [cost,grad_theta_tplushalf]= Chen_DeepDescTrainingCost_WithNonlinear_sigmoid_LRNNormalize(theta_tplusahalf,shrinkRate,x_l_Adjust,x_r_Adjust,y_adjust,3,10);
        
        % this comment part code is used to conduct the rmsprop with
%         nestrov gradient descent.
%         r_tminus1 = r_t; % r(t-1) = last r(t)
%         theta_tplusahalf = theta - beta_momentum.*v_t;
%         [cost,grad_theta_tplushalf]= Chen_DeepDescTrainingCost_WithNonlinear_sigmoid_LRNNormalize(theta_tplusahalf,shrinkRate,x_l,x_r,y,1,10);
...
%     hold on;
    ...
    [valid_Eucl_dist]= Chen_DeepDescTest_WithNonlinear_sigmoid_LRNNormalize(theta,shrinkRate,test_x_l,test_x_r,valid_test_y,3,10);
...
[test_Eucl_dist]= Chen_DeepDescTest_WithNonlinear_sigmoid_LRNNormalize(theta,shrinkRate,test_x_l,test_x_r,test_y,3,10);
...
test_y(1:Size_testSamples)=1;
test_y(Size_testSamples+1:2*Size_testSamples)=0;
[test_Eucl_dist]= Chen_DeepDescTest_WithNonlinear_sigmoid_LRNNormalize(theta,shrinkRate,test_x_l,test_x_r,test_y,1,10);
test_y(1:Size_testSamples)=1;
test_y(Size_testSamples+1:2*Size_testSamples)=-1;
vl_roc(test_y', -test_Eucl_dist(1:2*Size_testSamples)') ;
...
vl_roc(test_y', test_Eucl_dist(1:2*Size_testSamples)') ;
% clear TPR FPR
save('learnedParamaters0204_30_batchsize_1000_train_om_liberty_Yosimate_validation_withMining.mat','theta');
save('Save_v_t_r_t_Loss_alpha_0204_30_batchsize_1000__liberty_Yosimate_validation_withMining.mat','r_t','v_t','Loss_Rec','alpha');