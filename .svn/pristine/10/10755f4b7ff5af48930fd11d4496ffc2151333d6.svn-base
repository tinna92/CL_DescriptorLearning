...
function [affine_patch_pos,affine_patch_neg]=Chen_extractedaffinedpatchesfromOneimage_func(image_name,feature_detector_type,n,a,b,similarity_threshold)
...
if (strcmp(feature_detector_type,'fast12')~=1)&&(strcmp(feature_detector_type,'mser')~=1)
    ...
    [frames_origin, descrs_origin] = vl_covdet(A_Image, 'Method',feature_detector_type,'descriptor', 'Patch','PatchResolution',31,'Doubleimage',false,'Verbose','EstimateAffineShape', false,'EstimateOrientation', false) ;
else
...
for ii=1:n
    for jj=1:k(ii)
        ...
        if (strcmp(feature_detector_type,'fast12')~=1)&&(strcmp(feature_detector_type,'mser')~=1)
            ...
            [frames, descrs] = vl_covdet(current_affimg, 'Method',feature_detector_type,'descriptor', 'Patch','PatchResolution',31,'Doubleimage',false,'Verbose','EstimateAffineShape', false,'EstimateOrientation', false) ;
        else
            if strcmp(feature_detector_type,'fast12')==1
...
%         [index_ifrepeat_1stimg,repeated_feature_index_2ndimg] = Chen_analy_repeatedfeatures_in_2Images2(frames_origin, affine_frames,tform.T,similarity_threshold);
        [index_ifrepeat_1stimg,repeated_feature_index_2ndimg] = Chen_analy_repeatedfeatures_in_2Images2_Kdtree(frames_origin, affine_frames,tform.T,similarity_threshold);
%         [index_ifrepeat_1stimg,repeated_feature_index_2ndimg] = Chen_analy_repeatedfeatures_in_2Images2(frames_origin, affine_frames,tform.T,similarity_threshold);
