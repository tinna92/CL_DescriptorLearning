...
% alpha =  0.00003;
% alpha =  0.00003;
alpha =  3*alpha; % test what will happen
alpha_decreaserate_eachEpoch = 0.99;% how learning rate changes in every epoch
beta_momentum = 0.90;
theta = theta;
alpha =  0.00003*(0.99.^previous_epoch);
x_pos = notredame_x_64_pos;
x_neg = notredame_x_64_neg;


Num_Epoches  = 3; % num of iterations( for each training sample)
Size_batches = 250; % size of the batch
Num_batches = 1000; % how many batches are used for training
...
for iii=previous_epoch+1:previous_epoch+Num_Epoches
    ...
    for batch_ground =1:Num_batches 
        ...
        [cost,grad]= Chen_DeepDescTrainingCost_WithNonlinear_sigmoid_4layer(theta,shrinkRate,x_l,x_r,y,3,10); % test with nonlinear here
...
%         theta = theta - v_tplus1;  
        ...
        [cost,grad_theta_tplushalf]= Chen_DeepDescTrainingCost_WithNonlinear_sigmoid_4layer(theta_tplusahalf,shrinkRate,x_l,x_r,y,3,10);
