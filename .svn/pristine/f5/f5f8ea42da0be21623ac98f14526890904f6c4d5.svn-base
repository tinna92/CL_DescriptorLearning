...
function [index_ifrepeat_1stimg,repeated_feature_index_2ndimg] = Chen_analy_repeatedfeatures_in_2Images2_Kdtree(frames_origin, frames_aff,aff_T,similarity_threshold)
% load('Test_Coord.mat');
...
frames_origin_1 = frames_origin(1:2,:);
newfeatures_1 = frames_aff(1:2,:);

kdtree1 = vl_kdtreebuild(newfeatures_1) ;
[index, distance] = vl_kdtreequery(kdtree1, newfeatures_1, frames_origin_1) ;
test_index = find(distance(1,:)<1.0); % why get different result
repeated_feature_index_2ndimg = index(1,test_index);
index_ifrepeat_1stimg = distance(1,:)<1.0;
