...
% -------------------------------------------------------------------------
Num_Epoches  = 60; % num of iterations( for each training sample)
...
%             rate = 0.02;
            ...
            % if the current loss is larger than before, we decrease the
            % learning rate


   
            
%             Initiallearningrate = 3* Initiallearningrate;
%             res.ratew1 = Initiallearningrate*ones(size(w1));
%             res.ratew2 = Initiallearningrate*ones(size(w2));
%             res.ratew3 = Initiallearningrate*ones(size(w3));
%             res.rateb1 = Initiallearningrate*ones(size(b1));
...
%           L = y.*(max(0,Eucl_dist-1.5))+3*(1-y).*(max(0,4.5-Eucl_dist))+ 0.5 * shrinkRate * sum(w1(:).^2)+0.5 * shrinkRate * sum(w2(:).^2) +0.5 * shrinkRate * sum(w3(:).^2);% Try to change the loss function into L1 Norm
          ...
          L_sparsity(iii) = 0.5 * shrinkRate * sum(w1(:).^2)+0.5 * shrinkRate * sum(w2(:).^2) +0.5 * shrinkRate * sum(w3(:).^2);
...
%          Prop_gradtoW1 = mean(res.ratew1(:))*mean(res.gainw1(:))*mean(res.dzdw1(:))/mean(w1(:));
%          Prop_gradtoW2 = mean(res.ratew2(:))*mean(res.gainw2(:))*mean(res.dzdw2(:))/mean(w2(:));
%          Prop_gradtoW3 = mean(res.ratew3(:))*mean(res.gainw3(:))*mean(res.dzdw3(:))/mean(w3(:));
%          Prop_gradtoW1 = abs(0.001/Prop_gradtoW1);
%          Prop_gradtoW2 = abs(0.001/Prop_gradtoW2);
%          Prop_gradtoW3 = abs(0.001/Prop_gradtoW3);
%          res.ratew1 = Prop_gradtoW1*res.ratew1;
%          res.ratew2 = Prop_gradtoW2*res.ratew2;
%          res.ratew3 = Prop_gradtoW3*res.ratew3;
...
%          b_momentum3 =  rate * res.dzdb3 ;
    ...
    if iii >= 5
          if (Loss_Rec(iii) - Loss_Rec(iii-1))>0
              Initiallearningrate =  0.98*Initiallearningrate;
          else
              % if this time the change of cost is small than 0.01 times
              % whole cost, then turn up the learning rate
              if L_sparsity(iii)> (Loss_Rec(iii)-L_sparsity(iii))
                  Initiallearningrate =  0.98*Initiallearningrate;
              else
                  if (Loss_Rec(iii-1)-Loss_Rec(iii))/Loss_Rec(iii-1) < 0.01
                      Initiallearningrate = 1.3* Initiallearningrate;
                  end
              end
          end
