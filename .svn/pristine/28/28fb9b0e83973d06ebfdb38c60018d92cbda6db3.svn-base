...
% load('img1.ppmneg.mat');
% load('img1.ppmpos.mat');
% load('notredame_x_64_pos.mat');
% load('notredame_x_64_neg.mat');
x_pos = notredame_x_64_pos;
x_neg = notredame_x_64_neg;
% x_neg = affine_patch_neg;
% x_pos = affine_patch_pos;
Num_Pos_valid_set = 10000;% number of positive or negative samples in validation sets
...
% a = sqrt(6/4096);
% w1 = -a +2*a*(rand(5, 5, 1,3));
% a = sqrt(6/900);
% w2 = -a +2*a*(rand(5, 5, 3,9));
% a = sqrt(6/169);
% w3 = -a +2*a*(rand(4, 4, 9,27));
% a = sqrt(6/25);
% w4 = -a +2*a*(rand(5, 5, 27,81));
...
b1 = single(zeros(1,3));
b2 = single(zeros(1,9));
b3 = single(zeros(1,27));
b4 = single(zeros(1,81));
% rate = 5 ;
rate = 0.05;
momentum = 0.9 ;
% shrinkRate = 0.0001 ;
...
alpha_decreaserate_eachEpoch = 1;
...
w_momentum4 = zeros('like', w4) ;
b_momentum4 = zeros('like', b4) ;
% -------------------------------------------------------------------------
...
% alpha =  0.00003;
alpha =  0.0003;
alpha_decreaserate_eachEpoch = 1;% how learning rate changes in every epoch
beta_momentum = 0.90;
% sizew1 = [5 5 1 5];
% sizeb1 = [1 5];
% sizew2 = [5 5 5 25];
...
theta = [reshape(w1,1,totalsizew1(4)) reshape(b1,1,totalsizeb1(2)) reshape(w2,1,totalsizew2(4))...
    reshape(b2,1,totalsizeb2(2)) reshape(w3,1,totalsizew3(4)) reshape(b3,1,totalsizeb3(2))...
     reshape(w4,1,totalsizew4(4)) reshape(b4,1,totalsizeb4(2))];
...
for iii=1:Num_Epoches
    ...
    for batch_ground =1:Num_batches 
        ...
        [cost,grad]= Chen_DeepDescTrainingCost_WithNonlinear_sigmoid_4layer(theta,shrinkRate,x_l,x_r,y,0,10); % test with nonlinear here
...
%         theta = theta - v_tplus1;  
        ...
        [cost,grad_theta_tplushalf]= Chen_DeepDescTrainingCost_WithNonlinear_sigmoid_4layer(theta_tplusahalf,shrinkRate,x_l,x_r,y,0,10);
       ...
       % if the gradient and the accumulated velocity have the same
        % direction, then the current learning rate could be amplified, other
        % wise it should be shrinked
        sign_grad_theta_tplushalf = sign(grad_theta_tplushalf);
        sign_v_t = sign(v_t);
        same_index = sign_grad_theta_tplushalf==sign_v_t;
        amplex=1.05*same_index;
        differen_index = (sign_grad_theta_tplushalf~=sign_v_t);
        decrease = 0.95*differen_index;
        change_factor = amplex+decrease;
        alpha = alpha.*change_factor;
        
        % if it is too large, then truncated them...
        larger_index = alpha>20*initial_alpha;
        if(sum(larger_index)>0)
            alpha(larger_index)=20*initial_alpha;
        end        
        smaller_index = alpha<0.05*initial_alpha;
        if(sum(smaller_index)>0)
            alpha(smaller_index)=0.05*initial_alpha;
        end
...
% load('notredame_x_pos.mat');
% load('notredame_x_neg.mat');
load('liberty_x_64_pos.mat');
load('liberty_x_64_neg.mat');
x_neg = liberty_x_64_neg;
x_pos = liberty_x_64_pos;
...
[test_Eucl_dist]= Chen_DeepDescEuclideanDistance_WithNonlinear_sigmoid_4layer(theta,shrinkRate,test_x_l,test_x_r,test_y,3,10);
...
% clear TPR FPR
% save('learnedParamaters1112_80_graf.mat','theta');
