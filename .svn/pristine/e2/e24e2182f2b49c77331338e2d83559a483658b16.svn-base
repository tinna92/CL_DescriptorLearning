...
for ii=1:Size_trainingSamples
    x_pos{ii,1} = yosemite_x_pos{Training_sampleIndex_pos(ii),1};
    x_pos{ii,2} = yosemite_x_pos{Training_sampleIndex_pos(ii),2};
    x_neg{ii,1} = yosemite_x_neg{Training_sampleIndex_neg(ii),1};
    x_neg{ii,2} = yosemite_x_neg{Training_sampleIndex_neg(ii),2};
end

% % calculate the input for validation set
for ii=1:Size_validationSamples
    test_x_l(:,:,1,ii) = single(yosemite_x_pos{Validation_SampleIndex_pos(ii),1});
    test_x_r(:,:,1,ii) = single(yosemite_x_pos{Validation_SampleIndex_pos(ii),2});
    test_x_l(:,:,1,ii+Size_validationSamples) = single(yosemite_x_neg{Validation_SampleIndex_neg(ii),1});
    test_x_r(:,:,1,ii+Size_validationSamples) = single(yosemite_x_neg{Validation_SampleIndex_neg(ii),2});
end
