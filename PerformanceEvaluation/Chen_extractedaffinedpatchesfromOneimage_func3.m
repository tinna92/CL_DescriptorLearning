...
function [affine_patch_pos,affine_patch_neg]=Chen_extractedaffinedpatchesfromOneimage_func3(image_name,feature_detector_type,n,a,b,similarity_threshold,min_featurerepeattimes)
...
if (strcmp(feature_detector_type,'fast12')~=1)&&(strcmp(feature_detector_type,'mser')~=1)
    ...
    [frames_origin, descrs_origin] = vl_covdet(A_Image, 'Method',feature_detector_type,'descriptor', 'Patch','PatchResolution',31,'Doubleimage',true,'Verbose','EstimateAffineShape', false,'EstimateOrientation', true) ;
else
...
for ii=1:n
    for jj=1:k(ii)
        ...
        if (strcmp(feature_detector_type,'fast12')~=1)&&(strcmp(feature_detector_type,'mser')~=1)
            ...
            [frames, descrs] = vl_covdet(current_affimg, 'Method',feature_detector_type,'descriptor', 'Patch','PatchResolution',31,'Doubleimage',true,'Verbose','EstimateAffineShape', false,'EstimateOrientation', true) ;
        else
            if strcmp(feature_detector_type,'fast12')==1
